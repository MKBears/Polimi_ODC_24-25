...
+ **Open?Write** [25/01/10 exam]: [spoiler] how to use mmap syscall instead of read to get the content of a file to memory
...
+ **Playground**: Basic arbitrary chunk allocation attack with T-cache, heap boundary mechanism evasion
+ **ASCII Gallery**: Like Playground, but leaking stuff through malloc over freed chunk and method to avoid segmentation fault generated by heap corruption
...
+ **Estatea** [25/01/10 exam]: Functions dynamically wrote in memory and then called, but no need of dumping them. Signal handling in libdebug
